...
from models.spike_modules import Quant, DownSampling, UpSampling, MS_SpikeConvNextBlock
from models.rev_function import SpikeReverseFunction, ReverseFunction
...
class Fusion(nn.Module):
    def __init__(self, level, channels, first_col) -> None:
        ...
        if not first_col and level != 3:
            ...
            # self.tau = (
            #     nn.Parameter(
            #         shortcut_scale_init_value * torch.ones((1, channels[level], 1, 1)),
            #         requires_grad=True,
            #         # requires_grad=False,
            #     )
...
class MS_SpikeConvNextBlock_Reuse(nn.Module):
...
class MS_PatchEmbed_DVS(nn.Module):
    def __init__(self, channels, num_subnet) -> None:
        super().__init__()
        self.num_subnet = num_subnet
...
class FullNet(nn.Module):
    ...
    def forward(self, x):
        if self.dvs:
            return self._forward_intermediate_supervision_dvs(x)
        else:
            return self._forward_intermediate_supervision(x)

    def _forward_intermediate_supervision_dvs(self, imgs):
        x_cls_out = []
        c0, c1, c2, c3 = 0, 0, 0, 0
    ...
    def _forward_intermediate_supervision(self, img):
        x_cls_out = []
        c0, c1, c2, c3 = 0, 0, 0, 0
