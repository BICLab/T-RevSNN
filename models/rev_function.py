...
class ReverseFunction(torch.autograd.Function):
    ...
    def backward(ctx, *grad_outputs):
        ...
        ), torch.cuda.amp.autocast(**ctx.gpu_autocast_kwargs), torch.cpu.amp.autocast(
            **ctx.cpu_autocast_kwargs
        ):
            g3_up = g3_right
...
class SpikeReverseFunction(torch.autograd.Function):
    @staticmethod
    def forward(ctx, run_functions, *args):
        l0, l1, l2, l3 = run_functions
        ctx.run_functions = run_functions
        ctx.preserve_rng_state = True
    ...
    def backward(ctx, *grad_outputs):
        ...
        with torch.enable_grad(), torch.random.fork_rng(
            ...
            **ctx.cpu_autocast_kwargs
            ...
            g3_up = g3_right
            g3_left = g3_up  ## shortcut
            set_device_states(ctx.cpu_states_3, ctx.gpu_devices, ctx.gpu_states_3)
            oup3 = l3(c2, None)
            torch.autograd.backward(oup3, g3_up, retain_graph=True)
            with torch.no_grad():
                c3_left = c3 - oup3  ## feature reverse
            g2_up = g2_right + c2.grad
            g2_left = g2_up  ##shortcut
            ...
            with torch.no_grad():
                c2_left = c2 - oup2  ## feature reverse
            g3_left = g3_left + c3_left.grad if c3_left.grad is not None else g3_left
            g1_up = g1_right + c1.grad
            g1_left = g1_up  ##shortcut
            ...
            with torch.no_grad():
                c1_left = c1 - oup1  ## feature reverse
            g0_up = g0_right + c0.grad
            g0_left = g0_up  ##shortcut
            g2_left = (
                g2_left + c2_left.grad if c2_left.grad is not None else g2_left
            )  ## Fusion
            ...
            with torch.no_grad():
                c0_left = c0 - oup0  ## feature reverse
            gx_up = x.grad  ## Fusion
            g1_left = (
                g1_left + c1_left.grad if c1_left.grad is not None else g1_left
            )  ## Fusion
        ...
        if ctx.first_col:
            return None, gx_up, None, None, None, None
        else:
            return None, gx_up, g0_left, g1_left, g2_left, g3_left
