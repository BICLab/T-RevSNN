...
def gauss(x, a, b, c):
    """gauss"""
    return ops.exp(-ops.pow(ops.add(x, -b), 2).div(2 * c * c)).mul(a)
...
def colorize(x):
    ...
    if x.dim() == 2:
        ...
        x = ops.unsqueeze(x, 0)
    if x.dim() == 3:
        cl = ops.zeros([3, x.size(1), x.size(2)])
        cl[0] = gauss(x, .5, .6, .2) + gauss(x, 1, .8, .3)
        cl[1] = gauss(x, 1, .5, .3)
        cl[2] = gauss(x, 1, .2, .3)
        cl[cl.gt(1)] = 1
    elif x.dim() == 4:
        cl = ops.zeros([x.size(0), 3, x.size(2), x.size(3)])
        cl[:, 0, :, :] = gauss(x, .5, .6, .2) + gauss(x, 1, .8, .3)
        cl[:, 1, :, :] = gauss(x, 1, .5, .3)
        cl[:, 2, :, :] = gauss(x, 1, .2, .3)
    else:
        raise ValueError("colorize is wrong")
    return cl
